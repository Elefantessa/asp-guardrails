...
# ── Policy vocabulary hints ───────────────────────────────────────────────────
# Loaded from policies/vocabulary_map.json if present (generated by
# scripts/generate_vocabulary.py during ingestion). Falls back to hardcoded.

_VOCAB_HINT_FALLBACK = """
...
def _load_vocab_hint() -> str:
    ...
    if vocab_path.exists():
        try:
            ...
            if terms:
                lines = ["Key policy terms and their common user paraphrases:"]
                for term, paraphrases in terms.items():
                    phrase_list = ", ".join(f'"{p}"' for p in paraphrases[:6])
                    lines.append(f'- "{term}"\n    ← user says: {phrase_list}')
                return "\n".join(lines)
        except Exception:
            pass
    return _VOCAB_HINT_FALLBACK


_VOCAB_HINT = _load_vocab_hint()
...
def _rule_based_rewrite(query: str) -> str | None:
    ...
    # Cancellation fee queries — must fire BEFORE eligibility rule because complex
    # queries can contain both "cancel" and "lead booker" (e.g. TC031 multi-party
    # family cancellation). Extract days if present for a maximally specific query.
    # ISS-016: TC031 "what percentage will we lose" fell through to LLM rewriter
    # which retrieved company compensation table instead of customer fee table.
    _cancel_signals = [
        "cancel", "cancelling", "cancellation", "lose", "losing",
        "percentage will we", "percentage of the holiday cost",
    ]
    _day_signals = ["days before", "days before we", "days before departure"]
    if any(p in q for p in _cancel_signals) and any(p in q for p in _day_signals):
        days_match = _re.search(r'(\d+)\s*days?\s*before', q)
        if days_match:
            days = days_match.group(1)
            return f"customer cancellation fee percentage {days} days before departure"
        return "customer cancellation fee percentage days before departure"
